...
// Create subscription
// NOTE:jg - heres how we create a subscription. Would we check the users style preference here to send to survey or not? 
export async function create(parentValue, { crateId }, { auth }) {
  if(auth.user && auth.user.id > 0) {
    return await models.Subscription.create({
      crateId,
      userId: auth.user.id
      // NOTE:jg - My guess is that the "crateID" is passed from the function before and "userID" is being set
